...
public class CarService {
    ...
    public PageResponse<CarSet> selectPageEx(Integer offset, Integer pageSize) {
        PageResponse<CarSet> response = new PageResponse();
        List<CarSet> carSets = new ArrayList<>();
        List<Car> cars = carMapper.selectPage(offset, pageSize);

        for (Car car : cars) {
            CarSet carSet = new CarSet();
            carSet.setCar(car);
            carSet.setLicenses(licenseMapper.selectByCar(car.getId()));
            carSets.add(carSet);
        }
        response.setItem(carSets);
        response.setTotal(carMapper.count());
        response.setOffset(offset);
        response.setPageSize(pageSize);
        return response;
