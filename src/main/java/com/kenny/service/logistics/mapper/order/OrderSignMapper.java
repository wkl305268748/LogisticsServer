...
public interface OrderSignMapper{
	...
	@Insert("INSERT INTO tb_order_sign(fk_order_customer_id,order_img,time) VALUES(#{fk_order_customer_id},#{order_img},#{time})")
	@Options(useGeneratedKeys = true, keyProperty = "id")
	int insert(OrderSign orderSign);

	@Update("UPDATE tb_order_sign SET fk_order_customer_id=#{fk_order_customer_id},order_img=#{order_img},time=#{time} WHERE id=#{id}")
