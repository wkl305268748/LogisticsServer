...
import com.kenny.service.logistics.model.fleet.CarSet;
...
public class CarController{
	...
	public JsonBean<Car> Insert(@ApiParam(value = "车牌号如皖A5504",required = true)@RequestParam(value = "plate",required = true)String plate,
		...
		Car car = carService.insert(plate,type,resource,two_plate,driver_phone,driver_name,energy,length,weight,vin,brand,engine,axle,wheelbase,tire,factory_time,buy_time,buy_price,limited_time,tow_maintain_time,insurance_policy,insurance_company,insurance_time,front_img,tail_img,remark);
		licenseService.insertByCar(license,car.getId());
		return new JsonBean(ErrorCode.SUCCESS,car);
	}
	...
	@ResponseBody
	public JsonBean<PageResponse<CarSet>> selectPageEx(@ApiParam(value = "从第几个开始列出") @RequestParam(required = false, defaultValue = "0")Integer offset,
													 @ApiParam(value = "每页内容数量") @RequestParam(required = false, defaultValue = "10")Integer pageSize){
		return new JsonBean(ErrorCode.SUCCESS, carService.selectPageEx(offset,pageSize));
	}
