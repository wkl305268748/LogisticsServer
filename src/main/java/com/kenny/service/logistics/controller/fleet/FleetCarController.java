...
public class FleetCarController {
	...
	public JsonBean<FleetCar> Insert(@ApiParam(value = "用户token",required = true)@RequestParam(value = "token",required = true)String token,
                                     ...
                                     @ApiParam(value = "车辆证件",required = false)@RequestParam(value = "license[]",required = false)String license){

		try {
			User user = userBaseService.getUserByToken(token);
			FleetCar fleetCar = fleetCarService.insert(plate,type,resource,two_plate,driver_phone,driver_name,energy,length,weight,vin,brand,engine,axle,wheelbase,tire,factory_time,buy_time,buy_price,limited_time,tow_maintain_time,insurance_policy,insurance_company,insurance_time,front_img,tail_img,remark,user.getId());
	...
	@ResponseBody
	public JsonBean<FleetCar> update(@ApiParam(value = "查询主键", required = true)@PathVariable()Integer id,
                                     @ApiParam(value = "车牌号如皖A5504",required = true)@RequestParam(value = "plate",required = true)String plate,
                                     @ApiParam(value = "车辆类型如面包车",required = true)@RequestParam(value = "type",required = true)String type,
                                     @ApiParam(value = "车辆所属如自有车辆",required = true)@RequestParam(value = "resource",required = true)String resource,
                                     @ApiParam(value = "挂车车牌",required = false)@RequestParam(value = "two_plate",required = false)String two_plate,
                                     ...
                                     @ApiParam(value = "车辆备注",required = false)@RequestParam(value = "remark",required = false)String remark){
		try{
			return new JsonBean(ErrorCode.SUCCESS, fleetCarService.update(id,plate,type,resource,two_plate,driver_phone,driver_name,energy,length,weight,vin,brand,engine,axle,wheelbase,tire,factory_time,buy_time,buy_price,limited_time,tow_maintain_time,insurance_policy,insurance_company,insurance_time,front_img,tail_img,remark));
		}catch(ErrorCodeException e){
			return new JsonBean(e.getErrorCode());
		}
	}
	...
	@ResponseBody
	public JsonBean<FleetCar> selectByPrimaryKey(@ApiParam(value = "查询主键", required = true)@PathVariable()Integer id){
		try{
			return new JsonBean(ErrorCode.SUCCESS, fleetCarService.selectByPrimaryKey(id));
		}catch(ErrorCodeException e){
			return new JsonBean(e.getErrorCode());
		}
	}
	...
	@ResponseBody
	public JsonBean<PageResponse<FleetCar>> selectPage(@ApiParam(value = "用户token",required = true)@RequestParam(value = "token",required = true)String token,
		...
		try {
			User user = userBaseService.getUserByToken(token);
			return new JsonBean(ErrorCode.SUCCESS, fleetCarService.selectPageByBelongUser(offset,pageSize,user.getId()));
		} catch (ErrorCodeException e) {
			return new JsonBean(e.getErrorCode());
		}
	}

	@ApiOperation(value = "列出所有的Car带证件")
	@RequestMapping(value = "/ex/page",method = RequestMethod.GET)
	@ResponseBody
	public JsonBean<PageResponse<FleetCarSet>> selectPageEx(@ApiParam(value = "用户token",required = true)@RequestParam(value = "token",required = true)String token,
															@ApiParam(value = "从第几个开始列出") @RequestParam(required = false, defaultValue = "0")Integer offset,
                                                            @ApiParam(value = "每页内容数量") @RequestParam(required = false, defaultValue = "10")Integer pageSize){
		try {
			User user = userBaseService.getUserByToken(token);
			return new JsonBean(ErrorCode.SUCCESS, fleetCarService.selectPageByBelongUserEx(offset,pageSize,user.getId()));
		} catch (ErrorCodeException e) {
			return new JsonBean(e.getErrorCode());
		}
	}
	...
	public JsonBean deletePrimaryKey(@ApiParam(value = "查询主键", required = true)@PathVariable()Integer id){
		try {
			return new JsonBean(ErrorCode.SUCCESS, fleetCarService.deleteByPrimaryKey(id));
		} catch (ErrorCodeException e) {
			return new JsonBean(e.getErrorCode());
		}
	}
